...
c=$("#vk2LSHeaderContainer");2<=this.map.getZoom()&&this.getVisibility()?(b.html(this.features.length+" Messtischbl\u00e4tter gefunden."),c.hasClass("ui-state-error")&&c.removeClass("ui-state-error"),"visibilitychanged"==a.type&&this.refresh({force:!0})):(b.html("Bitte w\u00e4hlen Sie eine h\u00f6here Zoomstufe!"),c.addClass("ui-state-error"))},loadStarts:function(a){console.log("Loadstarts");$("#vk2LSHeaderLoadingContainer").addClass("loading")},loadEnds:function(a){console.log("Loadends");$("#vk2LSHeaderLoadingContainer").hasClass("loading")&&
$("#vk2LSHeaderLoadingContainer").removeClass("loading")},publishAddTimeLayerEvent:function(a,b,c,d){null!=b?4==a.length&&"number"===typeof parseInt(a)?(c.extent=d.getExtent(),c.time=a,b.publish("addtimelayer",c)):alert("W\u00e4hle validien Zeitstempel"):console.log("Missing publish/subscription handler")}},_loadToolbar:function(){var a=document.createElement("div");a.className+=" vk2LSToolPanel vk2LSToolbar";a.id="vk2LSToolbar";var b=document.createElement("div");b.className+=" vk2TimeSliderLabel1";
var c=document.createElement("div");c.className+=" vk2TimeSliderLabel2";var d=document.createElement("div");d.className+=" vk2TimeSliderDiv";var e=document.createElement("div");e.className+=" vk2AddLayerDiv";var f=document.createElement("input");f.className+=" vk2AddLayerInput";f.id="vk2AddLayerInput";f.type="text";var g=document.createElement("div");g.id="vk2AddLayerButton";g.innerHTML=VK2.Utils.get_I18n_String("displaytimestamp");e.appendChild(f);e.appendChild(g);a.appendChild(b);a.appendChild(c);
a.appendChild(d);a.appendChild(e);d=$(d).slider({range:!0,min:this._timestamps[0],max:this._timestamps[1],values:this._timestamps,animate:"slow",orientation:"horizontal",step:1,slide:function(a,d){$(b).text(d.values[0]);$(c).text(d.values[1])},change:$.proxy(function(a,b){this._updateFeatures(a,b.values)},this)});$(b).text(d.slider("values",0));$(c).text(d.slider("values",1));$(g).button().click($.proxy(function(a){a=$("#vk2AddLayerInput").val();this._eventListeners.publishAddTimeLayerEvent(a,this._PubSubHandler,
this._timeParameter,this._map)},this));return a},_loadHeader:function(){var a=document.createElement("div");a.className+=" vk2LSHeaderContainer ui-state-error";a.id="vk2LSHeaderContainer";var b=document.createElement("div");b.className+=" vk2LSHeaderContent";b.id="vk2LSHeaderContent";b.innerHTML="Bitte w\u00e4hlen Sie eine h\u00f6here Zoomstufe!";a.appendChild(b);b=document.createElement("div");b.className+=" vk2LSHeaderLoadingContainer";b.id="vk2LSHeaderLoadingContainer";a.appendChild(b);return a},
_loadContent:function(a,b){this._addTimeSearchLayer();this._featureStore=new GeoExt.data.FeatureStore({layer:this._timeLayer,features:this._features,fields:[{name:"mtbid",type:"string"},{name:"time",type:"string"},{name:"titel",type:"string"}]});var c=this._loadHeader();a.appendChild(c);var d=this._loadToolbar();a.appendChild(d);this._mainPanel=new Ext.Panel({renderTo:a.getAttribute("id"),id:"vk2LSMainPanel",cls:"vk2LSMainPanel",width:400,height:570,items:[{id:"vk2HeaderPanel",cls:"vk2HeaderPanel",
height:45,contentEl:c.getAttribute("id")},{xtype:"grid",width:400,height:400,store:this._featureStore,cm:new Ext.grid.ColumnModel([{id:"time",header:VK2.Utils.get_I18n_String("timestamp"),dataIndex:"time",sortable:!0},{id:"titel",header:VK2.Utils.get_I18n_String("titel"),dataIndex:"titel",sortable:!0}]),sm:new GeoExt.grid.FeatureSelectionModel({singleSelect:!1}),listeners:{rowclick:$.proxy(function(a,b,c){a=a.store.data.items[b];document.getElementById("vk2AddLayerInput").setAttribute("value",a.data.time)},
this),rowdblclick:$.proxy(function(a,b,c){this._map.setCenter(a.store.data.items[b].data.feature.bounds.getCenterLonLat(),9)},this),sortchange:function(a,b){console.log("Sort change event!")}},autoExpandColumn:"titel",id:"featureGridPanel",cls:"featureGridPanel"},{id:"vk2LSToolPanel",cls:"vk2LSToolPanel",contentEl:"vk2LSToolbar",width:400,height:130}],listeners:{afterlayout:$.proxy(function(){this._updateMapControlConfig()},this)}})},_addTimeSearchLayer:function(){this._features=new OpenLayers.Protocol.WFS({url:this._timeParameter.wfs,
...
e.length;f++)e[f].value==a.target.value&&(e[f].innerHTML=d.params.LAYERS+" "+c,d.oldTime=d.params.TIME,d.params.TIME=c,d.timeFtLayer.updateFeatures(),d.redraw(!0))},_createLayerElement:function(a){var b=this._createLayerDomElement(a);this._createEventBehaviorToLayerElem(b,a);return b},_createEventBehaviorToLayerElem:function(a,b){$(a).find("ul").hide();$(a).find(".minimize").find(".label").click(function(b){$(a).find("ul").slideToggle()});$(a).find(".olButton").click($.proxy(this.onButtonClick,this));
arguments={tooltipDiv:$(a).find(".opacity").find(".toolTip"),sliderDiv:$(a).find(".opacity").find(".sliderContainer"),value:100,min:0,max:100,step:1};this._addSliderBehaviour(arguments,this._opacitySliderEvent);if(b.isTime){arguments={tooltipDiv:$(a).find(".time").find(".toolTip"),sliderDiv:$(a).find(".time").find(".sliderContainer"),value:b.params.TIME,min:1868,max:1945,step:1};this._addSliderBehaviour(arguments,this._timeSliderEvent);var c=b.name+" "+b.params.TIME;$(a).find(".minimize").find(".label").html(c)}return b},
