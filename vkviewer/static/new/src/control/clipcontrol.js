goog.provide('VK2.Control.ClipControl');
 ...
 * 
 */
VK2.Control.ClipControl = function(clip_polygon_coordinates, clip_layer){
...
goog.inherits(VK2.Control.ClipControl, ol.control.Control);

/**
 * @private
 * @return {Array.<Array.<number>>}
 */
VK2.Control.ClipControl.prototype._getPixelForClipPolygon = function(){
	var map = this.getMap();
	var clip_pixel = [];
	for (var i = 0; i < this._rawCoordinates.length; i++){
		clip_pixel.push(map.getPixelFromCoordinate(this._rawCoordinates[i]));
	};
	return clip_pixel;
};
 ...
 * @private
 */
VK2.Control.ClipControl.prototype._drawClipPolygonOnCanvas = function(clip_pixel, canvas){
	canvas.beginPath();
 ...
 * @return boolean;
 */
VK2.Control.ClipControl.prototype._isExtentWithinClipPolygon = function(extent){
 ...
 */
VK2.Control.ClipControl.prototype.isPointInPolygon = function(poly, point){
