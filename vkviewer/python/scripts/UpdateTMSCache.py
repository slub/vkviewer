...
def buildTsmCache(source_path, target_dir, logger):
    ...
    command = 'gdal2tiles.py -z 1-15 -w none %s %s'%(source_path, tms_target_dir)
    # now execute command
...
def updateTMSCache(source_dir, target_dir, logger):
    ...
    for file in directory_content:
        buildTsmCache(os.path.join(source_dir, file), target_dir, logger)
